...
## 7-6. サブ枠・カード内テキストのはみ出し防止（2026-07 追加）

2026-07 に西表短期カードの英語ルート名／長期パネルの白背景／運休ラベルがカード・枠からはみ出す不具合が
複数発生した。以下を恒久ルールとする。

1. **英語ルート名に "Route" / "Line" を付けない**（`Uehara` であって `Uehara Route` ではない）。
   ピル（和名）＋英語を横並びで描くレイアウトは、英語が長いと枠右端を超えやすい。
2. **サブ枠・パネル内テキストの右端は、枠/カード右端から最低15pxのクリアランスを確保**する。
   左詰めラベル（船種名・`運休 Suspended`・`NN%`）は特に見落としやすい。運休(`suspended`)と最大値(`100%`)を
   含む worst-case で必ず目視確認する。
3. **可変部の白塗り（再描画）はテンプレのカード実測枠を超えない**。角丸カード上に `PANEL_BG` を塗り直す際、
   塗り矩形がテンプレのカード右端を越えると白がはみ出す（西表長期・大原パネルの実例：塗り右端1170 > カード右端1156）。
   塗り範囲はテンプレのカード内に収める。
4. **リスク期間が1日のときは単日表示**（`start==end` なら `7/26 〜 7/26` ではなく `7/26`）。

**実装リポと共通運用**：座間味＝`kucha-ferry-alert`、渡嘉敷＝`tokashiki-logger`、八重山（西表＝`iriomote_images.py` /
その他3島＝`others_images.py`）＝`yaeyama-all-routes-logger`。本仕様書（本リポ `docs/`）を全リポ共通の SSOT とする。
八重山リポには本ルールの回帰検査 `check_image_layout.py`（CI: `image_layout_check.yml`・push時にworst-caseで描画し
カード外への白はみ出しを検出）を実装済み。他リポの短期/長期レンダラを変更する際も本ルールを適用すること。
