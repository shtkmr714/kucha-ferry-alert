...
def detect_suspension_conflicts(scraped, manual):
    ...
    for m in manual:
        ...
        matching = [s for s in scraped if s.get("service") in (m_service, "both") or m_service == "both"]
        for s in matching:
            s_start = s.get("start", "")
            s_end = s.get("end", "")
...
def get_planned_suspensions():
    ...
    if merged:
        for sus in merged:
            print(f"    {sus.get('vessel_ja','')} {sus.get('service','')} {sus.get('start','')}〜{sus.get('end','')} {sus.get('reason_ja','')}")
    return merged
