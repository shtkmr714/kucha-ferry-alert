...
def make_image_weather_data(forecast, output_path):
    """画像④: 予報根拠データ（日英併記）。短期・長期と同じ青い海グラデ背景＋白カード。"""
    ...
    # タイトル（海グラデ上・白文字）
    draw.text((540, 62), "予報根拠データ  /  Forecast Data", font=f["title"], fill="white", anchor="mm")

    def section_header(y, ja, en):
        draw.rounded_rectangle([(60, y), (1020, y + 44)], radius=10, fill=(232, 238, 245))
        draw.text((80, y + 22), f"【{ja} / {en}】", font=f["sec_hd"], fill=(40, 60, 100), anchor="lm")
        return y + 60
    ...
    if suspensions:
        draw.rounded_rectangle([(60, y), (1020, y + 42)], radius=10, fill=(232, 245, 235))
        draw.text((80, y + 21), "【計画運休情報 / Scheduled Suspension】",
                  font=f["sec_hd"], fill=(46, 125, 50), anchor="lm")
        ...
        for sus in suspensions:
            ...
            draw.text((80, y),
                      ...
                      font=f["label_ja"], fill=(40, 100, 55), anchor="lm")
            draw.text((1010, y + 11), f"{s_label}〜{e_label}",
                      font=f["value"], fill=(46, 125, 50), anchor="rm")
            draw.text((96, y + 24), f"  {sus.get('vessel_en','—')}",
                      font=f["label_en"], fill=(110, 150, 120), anchor="lm")
            draw.text((1010, y + 30), f"({s_label_en} - {e_label_en})",
                      font=f["label_en"], fill=(110, 150, 120), anchor="rm")
            y += 62
            draw.text((80, y),
                      f"  理由: {sus.get('reason_ja','—')}  /  {sus.get('reason_en','—')}",
                      font=f["label_ja"], fill=(90, 110, 95), anchor="lm")
            draw.text((80, y + 23), "  出典: 座間味村HP（自動取得）",
                      font=f["label_en"], fill=(120, 140, 125), anchor="lm")
            y += 52
        y += 8
        draw.line([(60, y), (1020, y)], fill=(226, 230, 236), width=1)
        y += 8

    # 【台風情報】セクション（接近予報がある場合のみ）
    typhoon = forecast.get("typhoon")
    if typhoon:
        draw.rounded_rectangle([(60, y), (1020, y + 42)], radius=10, fill=(253, 236, 236))
        draw.text((80, y + 21), f"【台風情報 / Typhoon】 {typhoon['name_label']}",
                  font=f["sec_hd"], fill=(198, 40, 40), anchor="lm")
        ...
        for d in typhoon["days"][:6]:
            ...
            draw.text((80, y), f"・{d['date_label']}",
                      font=f["label_ja"], fill=(190, 60, 60), anchor="lm")
            draw.text((96, y + 24), f"  {d['date_label_en']}",
                      font=f["label_en"], fill=(200, 110, 110), anchor="lm")
            draw.text((1010, y), value_ja, font=f["value"], fill=(150, 40, 40), anchor="rm")
            draw.text((1010, y + 24), value_en, font=f["label_en"], fill=(200, 110, 110), anchor="rm")
            y += 52
        y += 6
        draw.line([(60, y), (1020, y)], fill=(226, 230, 236), width=1)
        y += 8
    ...
    draw.text((80, y), "気象庁（jma.go.jp）  /  Open-Meteo Marine API", font=f["src"], fill=(38, 48, 68), anchor="lm")
    draw.text((80, y + 30), "座間味村HP（vill.zamami.okinawa.jp）", font=f["src"], fill=(38, 48, 68), anchor="lm")
    draw.text((80, y + 60), "Zamami Village official site / JMA (jma.go.jp)", font=f["src"], fill=(120, 130, 150), anchor="lm")

    # フッター（海グラデ上・淡色）
    draw.text((540, 1038), "※欠航判断は船会社・座間味村が行います。本データはAI予測の参考値です。",
...
# 欠航時の那覇ホテル記事（慶良間＝泊港発のため座間味・阿嘉・渡嘉敷の共通導線）
...
def _naha_hotels_cta(forecast):
    ...
    try:
        vals = [v for d in forecast.get("short_term", [])[:2]
                for v in (d.get("highspeed_pct") or 0, d.get("ferry_pct") or 0)]
...
"""
    ...
    caption = resp.content[0].text
    return caption + _naha_hotels_cta(forecast)
