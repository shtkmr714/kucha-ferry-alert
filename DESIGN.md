...
### 4-2. 波高 → 欠航確率（波高単独ロジスティックモデル）

> **2026-06 波高単独モデルへ移行**。特徴量選択分析（過去165日）の結果、
> うねり・風速は波高と強く相関（風速 r=+0.84、うねり r=+0.65）し、
> 多変量ロジスティック回帰で波高を制御すると**いずれも有意でない**（p>0.1）。
> ネストモデル比較でも「波高のみ」と「波+うねり+風」のCV-AUCはほぼ同等
> （高速船 0.938 vs 0.949、フェリー 0.994 vs 0.997）。
> さらに events-per-variable が不足（フェリー 5.3）し、変数を絞る方が頑健。
> → **欠航%は波高(max_wave)の直接ロジスティック関数**とする。
> うねり・風速は重み配分から除外（ただし将来検証用にログ収集は継続）。

```python
pct = 100 / (1 + exp(-steepness × (wave_m - inflection_m)))
```

| 対象 | 変曲点 (50%到達波高) | 急峻さ | 根拠 |
|---|---|---|---|
| 高速船 | 2.01 m | 4.92 | 波高単独フィット（n=155 / 欠航34日, weather のみ） |
| フェリー | 2.68 m | 7.34 | 波高単独フィット（n=139 / 欠航16日）。耐波性が高く変曲点が約0.7m高い |
...
| 2.5 m | 92% | 100% | 21% | 78% |
| 3.0 m | 99% | 100% | 91% | 88% |
| 3.5 m | 100% | 100% | 100% | 100% |

> ※ 渡嘉敷（tokashiki）は運航判断基準が異なる（座間味より欠航しにくい）ため別パラメータ。
> ※ weather 欠航のみ対象（dock/equipment 除外）。
> ※ `cancellation_score`（波+うねり+風+注意報の合成スコア）は DB 継続記録のため
>   `calc_cancellation_score()` で従来通り算出・保存するが、**表示する欠航%の駆動には
>   使わない**（波高単独に置換）。将来うねり周期・風向・突風の増分価値を再検証する材料として温存。
> ※ 波高が実態に追いつかない局面（台風急接近等）は typhoon_floor / JMA早期注意情報で別途補完。
